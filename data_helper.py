...
def clean_data(data, min_utt_number, max_utt_length):
    ...
    data_mod6.to_csv("data/ev_thread.csv", index=False, header=['id', 'label', 'content'], columns=['dialogId', 'evaluation', 'thread'])
...
def data_to_emb(data_file_path, emb_file_path):
    ...
    data = np.zeros([len(threads), max_length])
    for th_idx, thread in enumerate(threads):
        for w_idx, word in enumerate(thread):
            data[th_idx, w_idx] = vocab.index(word)
    ...
    return ids, data, labels, lengths, vocab_embs
...
def batch_iter( data, labels, lengths, batch_size, num_epochs):
    ...
    for _ in range(num_epochs):
        for i in range(epoch_length):
            ...
            seq_length = lengths[start_index: end_index]
           

            yield xdata, ydata, seq_length


# In[16]:


def my_train_test_split(ids, data, labels, lengths, random_state=None, test_size=0.1):
    valid_i = []
    train_i = []
    id_dict = Counter(ids)
    ...
    lengths_train = lengths[train_i]
    lengths_valid = lengths[valid_i]

    
    return  data_train, data_valid, labels_train, labels_valid,lengths_train,lengths_valid  
